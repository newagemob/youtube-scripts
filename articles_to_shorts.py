...
def get_tech_articles():
    ...
    for article in articles:
        ...
        generate_summary(title=article['title'], prompt=prompt, directory='tech_video_scripts')
...
def get_programming_examples(language, method):
    ...
    for link in links[3:4]:
        try:
            ...
            paragraphs = soup.find_all('p')
            text = ' '.join([p.get_text() for p in paragraphs])
            generate_summary(title=f"{language} {method}", prompt=text, directory='programming_video_scripts')
        except:
            continue
...
def get_history_articles():
    ...
    for post in posts:
        if post != '':
            ...
            generate_summary(title=post, prompt=prompt, directory='history_video_scripts')
            time.sleep(2)
